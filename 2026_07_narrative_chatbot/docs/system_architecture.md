...
7. `InterpretTurn`が確定した返答とやり取りから、物語アクション（`narrative_actions`）、決まったこと（`settled_details`）、返答の最後の問いかけの種類（`question_kind`: なし／案を深める／参加者自身のことを尋ねる／聞き違いの確認／新しい案を求める／決めるべきことを委ねる）を判断する。`none`以外なら答え待ちとして扱い、コンパイル時は「新しい案を求める」「委ねる」をhard gateで不採用にする。セリフは書かない。返答の生成と分けることで、この段のデモをすべて判断の例に使い、完成した文章を見て判断できる。Python処理がこれらを検証し、時間イベントと状態へ反映する。
